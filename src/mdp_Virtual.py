...
class ValueIterationPlanner(Planner):
    ...
    def show(self, A, V, state, map,     DIR, VIZL, VIZD, STATE_HISTORY):
        ...
        if viz:
            ...
            origin_x = -1
            origin_y = 1
            ax1.plot(origin_y, origin_x, ".y", markersize=10,     label = "Agent")
            ...
            try:
                ...
                ax1.plot(origin_y+(-DIR[2]+DIR[3])/max_dir, origin_x+(DIR[0]-DIR[1])/max_dir, marker = "*", color = "orange", markersize=10,     label = "Estimated") # , alpha = 0.5)
                ax1.plot([origin_y, origin_y+(-DIR[2]+DIR[3])/max_dir], [origin_x, origin_x+(DIR[0]-DIR[1])/max_dir], linestyle = "--", color='orange', alpha = 0.5)
            except:
                pass
